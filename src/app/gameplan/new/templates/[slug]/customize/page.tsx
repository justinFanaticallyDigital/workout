...
/**
 * R14 — Per-template customization wizard at
 * `/gameplan/new/templates/[slug]/customize`.
 *
 * Walks through the template's `customizationInputs` array, validating
 * each step, surfacing engine warnings on review, and POSTing to
 * /api/programs/clone (R13) on confirm. Routes to /gameplan/{programId}
 * on success.
 *
 * Draft persistence: stepIndex + answers persist to localStorage under
 * `picker-customize-${slug}-v1` (24h TTL via @/lib/draft-store).
...
export default function CustomizeTemplatePage({ params }: PageProps) {
  ...
  if (!template) {
    return <UnknownTemplate slug={slug} />;
  }

  return <CustomizeWizard slug={slug} template={template} router={router} toast={toast} />;
}
...
function CustomizeWizard({
  ...
  const draftKey = `picker-customize-${slug}-v1`;
  ...
  // R14 — restore draft on mount; save on every change. firstPaint
  // gate prevents the initial save from clobbering a stored draft.
  const firstPaint = useRef(true);
  useEffect(() => {
    const stored = loadDraft<PersistedDraft>(draftKey);
    if (stored) {
  ...
  useEffect(() => {
    ...
    saveDraft<PersistedDraft>(draftKey, { stepIndex, answers });
  }, [draftKey, stepIndex, answers]);
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(body?.error ?? "Could not create program");
        setSubmitting(false);
        return;
      }
      ...
      // R14 — surface applyCustomizations warnings as info toasts so
      // partial-failure passes (e.g. no bodyweight → skipped nutrition)
      // are visible to the user. Cleared draft + routed home.
      clearDraft(draftKey);
