...
import { PlanningGoalCard } from "./PlanningGoalCard";
import { FeasibilityWarning } from "./FeasibilityWarning";
import { DateRangeCard } from "./DateRangeCard";
import { PlanningBlockTimeline } from "./PlanningBlockTimeline";
...
import { GameplanKindEditor } from "./GameplanKindEditor";
import { computeDiff } from "./diff";
import { buildApplySteps, buildInverseSteps } from "./applyDiff";
import type {
  PlanningDraft,
  SectionTab,
  ProjectionTab,
 ...
 *      UndoToast + clear localStorage + redirect home.
 *   6. Discard → clear localStorage + route home.
 *   7. Reset → restore draft from `original` snapshot.
 *
 * Tier-aware (3.5): rendered by both the Gameplan route
 * (/gameplan/[id]/planning) and the Program route
 * (/programs/[programId]/planning). The tier comes from the global
 * TierProvider. The Program tier is the refactor branch (MIGRATION_MAP §C3) —
 * it drops the check-in audit layer (the GameplanChange write), the
 * recommendation banner, the recent-changes panel, the gameplan-kind editor
 * and the Lifestyle tab, and routes home to /my-program instead of /gameplan.
 */
export function PlanningExperience({ programId }: { programId: string }) {
  const router = useRouter();
  const toast = useToast();
  const { tier } = useTier();
  const isGameplan = tier === "gameplan";
  const homeHref = isGameplan ? "/gameplan" : "/my-program";
  const planLabel = isGameplan ? "gameplan" : "program";
  ...
  const parsedTab: SectionTab =
    initialTabRaw && (VALID_TABS as readonly string[]).includes(initialTabRaw)
      ? (initialTabRaw as SectionTab)
      : "goals";
  // Program tier has no Lifestyle tab — clamp a deep-link to Goals.
  const initialTab: SectionTab = !isGameplan && parsedTab === "lifestyle" ? "goals" : parsedTab;
  ...
  if (loadError) {
    return (
      <div className="min-h-screen bg-ft-bg text-ft-on-bg max-w-2xl mx-auto p-6">
        <Marker style={{ fontSize: 22, color: "rgb(var(--ft-danger-fg))" }}>Couldn&apos;t load {planLabel}</Marker>
  ...
  const handleConfirmApply = async (note: string) => {
    ...
    try {
      ...
      // apply session. The audit trail is a Gameplan-tier concern (the
      // check-in / recommendation ledger reads it); the Program tier
      // applies edits without it (MIGRATION_MAP §C3).
      if (isGameplan) {
        try {
  ...
  return (
    ...
    >
      ...
      {isGameplan && recommendationId && <RecommendationBanner recommendationId={recommendationId} />}

      <DiffSummaryStrip diff={diff} />
      <SectionTabs
        active={activeTab}
        onChange={setActiveTab}
        tabs={isGameplan ? undefined : ["goals", "timeline", "nutrition", "training"]}
      />
          ...
          {draft.goals.length === 0 ? (
            <div style={{ padding: "10px 12px" }}>
              <Archivo size={10} color="rgb(var(--ft-text-tertiary))">
                No goals on this {planLabel} yet. Add goals from the picker before editing.
              </Archivo>
            </div>
          ) : (
            draft.goals.map((g) => (
      ...
      {isGameplan && activeTab === "lifestyle" && (
        <>
          <div style={{ padding: "8px 12px 0" }}>
      ...
      {isGameplan && <RecentChangesPanel programId={programId} />}
...
function diffEntryLabel(d: import("./types").DiffEntry): string {
  switch (d.kind) {
    case "program":
