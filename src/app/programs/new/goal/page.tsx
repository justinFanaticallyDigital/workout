          ...
          {GOAL_TYPES.map((gt) => (
            <button key={gt.value} onClick={() => handleSelectType(gt.value)} className="text-left">
              <Card className={`h-full hover:border-ft-dim transition-colors ${goalType === gt.value ? "border-ft-white" : ""}`}>
                ...
                <h3 className="font-body text-sm font-bold mb-1">{gt.label}</h3>
                <p className="text-ft-dim text-xs font-body">{gt.desc}</p>
              </Card>
            </button>
          ))}
        </div>
      ...
      {step === 2 && (
        <Card>
          <div className="space-y-4">
            <div>
              <label className="block text-ft-dim text-xs font-body uppercase tracking-wider mb-1.5">
                Goal Title *
              </label>
              <input
                type="text"
                ...
                className="w-full bg-ft-bg border border-ft-card rounded px-3 py-2 text-sm font-body text-ft-white placeholder:text-ft-muted focus:outline-none focus:border-ft-dim"
              />
            </div>
            ...
            {(goalType === "weight" || goalType === "competition") && startValue && (
              <div className="flex items-center gap-2 p-2 bg-ft-bg rounded border border-ft-card">
                <span className="text-ft-dim text-xs font-body">Current weight:</span>
                <span className="text-ft-white text-sm font-body font-bold">{startValue} lbs</span>
                {startValueSource === "fitbit" && (
                  <span className="inline-flex items-center gap-1 text-[#00B0B9]">
                    <FitbitIcon size={10} color="#00B0B9" />
                    <span className="text-[10px] font-body">Fitbit</span>
                  </span>
                )}
              </div>
            )}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-ft-bg border border-ft-card rounded px-3 py-2 text-sm font-body text-ft-white focus:outline-none focus:border-ft-dim"
                  >
            ...
            <div className="flex justify-between pt-2">
              <button onClick={() => setStep(1)} className="text-ft-dim text-sm font-body hover:text-ft-light">
                &larr; Back
              </button>
              <button
                onClick={() => setStep(3)}
                disabled={!title.trim()}
                className="bg-ft-white text-ft-bg font-body text-sm font-bold px-4 py-2 rounded hover:bg-ft-light disabled:opacity-50"
              >
                Next &rarr;
              </button>
            </div>
        ...
        <Card>
          <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="w-full bg-ft-bg border border-ft-card rounded px-3 py-2 text-sm font-body text-ft-white focus:outline-none focus:border-ft-dim"
                  />
                  <span className="text-ft-dim text-xs font-body whitespace-nowrap">weeks</span>
                </div>
              </div>
            </div>
            ...
            <div className="p-3 bg-ft-bg border border-ft-border rounded">
              ...
              <p className="text-ft-light text-xs font-body mb-1">
            ...
            <div className="flex justify-between pt-2">
              <button onClick={() => setStep(2)} className="text-ft-dim text-sm font-body hover:text-ft-light">
                &larr; Back
              </button>
              <button
