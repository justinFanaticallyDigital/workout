...
export default function DayTemplatePage({
  ...
  const validateExerciseForm = (): boolean => {
    ...
    if (!selectedExercise) errors.exercise = "Select an exercise";
    const sets = parseInt(targetSets);
    if (targetSets && (isNaN(sets) || sets < 1 || sets > 20)) errors.sets = "1-20";
    if (targetRepRange.trim() && !/^\d+(-\d+)?$/.test(targetRepRange.trim())) {
      errors.repRange = "Format: 8 or 8-12";
    }
    if (targetRpe.trim()) {
      const rpeMatch = targetRpe.trim().match(/^(\d+(?:\.\d+)?)(?:-(\d+(?:\.\d+)?))?$/);
      if (!rpeMatch) {
        errors.rpe = "Format: 7 or 7-9";
      } else {
        const low = parseFloat(rpeMatch[1]);
        const high = rpeMatch[2] ? parseFloat(rpeMatch[2]) : low;
        if (low < 1 || high > 10) errors.rpe = "Range: 1-10";
      }
    }
  ...
  if (!day) {
    return (
      <div className="min-h-screen bg-ft-bg text-ft-white p-6">
        <p className="text-ft-light font-body">Day not found.</p>
      </div>
    );
  }
            ...
            <div>
              ...
              {selectedExercise ? (
                <div className="flex items-center gap-2 bg-ft-bg border border-ft-card rounded px-3 py-2">
                  <span className="text-ft-white text-sm font-body flex-1">
                    ...
                    <div className="absolute top-full left-0 right-0 mt-1 bg-ft-surface border border-ft-card rounded max-h-48 overflow-y-auto z-10">
                      {searchResults.map((ex) => (
                        <button
                          ...
                          className="w-full text-left px-3 py-2 text-sm font-body text-ft-light hover:bg-ft-card transition-colors"
                        >
                          {ex.name}
                          {ex.primaryMuscle && (
            ...
            <div className="grid grid-cols-4 gap-3">
              <div>
                ...
                <input
                  type="number"
                  value={targetSets}
                  onChange={(e) => { setTargetSets(e.target.value); setFormErrors((p) => { const n = { ...p }; delete n.sets; return n; }); }}
                  min="1"
                  max="20"
                  className={`w-full bg-ft-bg border rounded px-3 py-2 text-sm font-body text-ft-white focus:outline-none focus:border-ft-dim transition-colors ${formErrors.sets ? "border-ft-danger" : "border-ft-card"}`}
                />
                {formErrors.sets && <span className="text-ft-danger text-[10px] font-body">{formErrors.sets}</span>}
              </div>
              <div>
                <label className="block text-ft-dim text-xs font-body uppercase tracking-wider mb-1">
                  Rep Range
                ...
                <input
                  ...
                  onChange={(e) => { setTargetRepRange(e.target.value); setFormErrors((p) => { const n = { ...p }; delete n.repRange; return n; }); }}
                  placeholder="8-12"
                  className={`w-full bg-ft-bg border rounded px-3 py-2 text-sm font-body text-ft-white placeholder:text-ft-muted focus:outline-none focus:border-ft-dim transition-colors ${formErrors.repRange ? "border-ft-danger" : "border-ft-card"}`}
                />
                {formErrors.repRange && <span className="text-ft-danger text-[10px] font-body">{formErrors.repRange}</span>}
              </div>
              <div>
                <label className="flex items-center gap-1.5 text-ft-dim text-xs font-body uppercase tracking-wider mb-1">
                  RPE
                ...
                <input
                  type="text"
                  value={targetRpe}
                  onChange={(e) => { setTargetRpe(e.target.value); setFormErrors((p) => { const n = { ...p }; delete n.rpe; return n; }); }}
                  placeholder="e.g. 7-8"
                  className={`w-full bg-ft-bg border rounded px-3 py-2 text-sm font-body text-ft-white placeholder:text-ft-muted focus:outline-none focus:border-ft-dim transition-colors ${formErrors.rpe ? "border-ft-danger" : "border-ft-card"}`}
                />
                {formErrors.rpe && <span className="text-ft-danger text-[10px] font-body">{formErrors.rpe}</span>}
              </div>
              <div>
                <label className="flex items-center gap-1.5 text-ft-dim text-xs font-body uppercase tracking-wider mb-1">
                  Progression
            ...
            <Card key={bde.id}>
              <div className="flex items-start justify-between">
                <div className="flex items-start gap-3">
                  <span className="text-ft-muted text-xs font-body font-bold mt-0.5 w-5">
                    {idx + 1}.
                  </span>
                  <div>
                    <h3 className="font-body text-sm font-bold mb-1">
                      {bde.exercise.name}
                    </h3>
                    <div className="flex items-center gap-3 text-xs font-body text-ft-dim">
                    ...
                    {bde.altExercise && (
                      <p className="text-ft-muted text-xs font-body mt-1">
                ...
                <div className="mt-3 pt-3 border-t border-ft-border ml-8">
                  {bde.progressionType !== "none" && (
                    <div className="flex items-center gap-2">
                      <span className="text-ft-muted text-[10px] font-body uppercase tracking-wider">
                        Progression
                      </span>
                ...
                <Link key={w.id} href={`/history/${w.id}`}>
                  <Card className="hover:border-ft-dim transition-colors">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-4 text-ft-dim text-xs font-body">
                        <span>{sets} sets</span>
