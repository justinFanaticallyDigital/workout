...
function ThemeHeader({ theme, session }) {
  ...
  if (theme === 'notebook') {
    return (
      <div className="ft-header" style={{ padding: '18px 16px 10px 52px', position: 'relative' }}>
...
function WeekStrip({ theme, session, weekIdx, setWeekIdx }) {
  const themeStyles = {
    iron: {
      bg: 'transparent',
        ...
        return (
          <button
            ...
            style={{
              ...
              border: isActive ? s.activeBorder : '1px solid transparent',
  ...
  return (
    <div style={{
      ...
      <span className="ft-data" style={{
        ...
        {lane.targetSets}×{lane.targetReps}
      </span>
    </div>
  );
}
...
function SetCell({ theme, set, prev, setIdx, lastWeekMode, isActive, cat, onTap, dense }) {
  ...
  const base = {
    ...
    border: `1px solid ${isActive ? 'rgb(var(--ft-accent))' : 'rgba(var(--ft-border),.55)'}`,
  ...
  if (filled) {
    ...
    base.borderColor = 'rgba(var(--ft-accent),.6)';
  }
  if (isActive) {
      ...
      {filled ? (
        <>
          ...
          <div className="ft-data" style={{
            ...
            color: 'rgb(var(--ft-dim))',
            marginTop: theme === 'arcade' ? 2 : 1,
          }}>×{set.r}{!dense && set.rpe ? ` @${set.rpe}` : ''}</div>
        </>
      ) : showGhost ? (
        <>
          <div className="ft-data" style={{
            fontSize: wFontGhost,
            lineHeight: 1,
            color: 'rgba(var(--ft-muted),.9)',
            fontWeight: 500,
            fontStyle: theme === 'notebook' ? 'italic' : 'normal',
          }}>{fmtW(prev.w)}</div>
          <div className="ft-data" style={{
            fontSize: rFontGhost, color: 'rgba(var(--ft-muted),.7)', marginTop: 1,
          }}>×{prev.r}</div>
        </>
      ) : (
        <div style={{
...
function AddSetCell({ theme, onTap, dense }) {
  return (
    <button onClick={onTap} style={{
      flex: 1,
      aspectRatio: dense ? '1 / 1' : '1 / 0.85',
      minHeight: dense ? 46 : 52, minWidth: dense ? 28 : 36,
      position: 'relative', cursor: 'pointer',
      background: 'transparent',
      border: '1px dashed rgb(var(--ft-border) / .7)',
      borderRadius: 'var(--ft-radius)',
      display: 'flex', flexDirection: 'column',
      alignItems: 'center', justifyContent: 'center',
      padding: dense ? 1 : 2, color: 'rgb(var(--ft-muted))',
    ...
    onMouseLeave={(e) => {
      ...
      e.currentTarget.style.borderColor = 'rgb(var(--ft-border) / .7)';
      e.currentTarget.style.color = 'rgb(var(--ft-muted))';
    }}>
      <div style={{ fontSize: 18, lineHeight: 1, fontWeight: 400 }}>+</div>
      <div className="ft-data" style={{
        fontSize: 8, letterSpacing: '.12em',
        textTransform: theme === 'notebook' ? 'lowercase' : 'uppercase',
        opacity: .8, marginTop: 2,
      }}>{theme === 'notebook' ? 'add' : 'SET'}</div>
    </button>
  );
}
...
function LoggerScreen({ theme, session = window.SESSION, tweaks = {} }) {
  ...
  return (
    <div className="ft" data-theme={theme} style={{
      ...
      {/* theme-specific page chrome (spiral binding for notebook, crt for arcade) */}
      {theme === 'notebook' && <div className="ft-page-chrome" />}
      ...
      <div className="ft-on-bg" style={{
        ...
        <WeekStrip theme={theme} session={localSession} weekIdx={weekIdx} setWeekIdx={setWeekIdx} />
        <div style={{ padding: theme === 'notebook' ? '0 18px 0 50px' : '0 14px' }}>
