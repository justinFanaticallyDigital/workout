...
// ═══════════════════════════════════════════════════════════════
// SCREEN — 4.5 Goals Editor (standalone) · in Planning Mode
// The dedicated surface to add / edit / prioritize / archive goals — the
// goal handles that otherwise only live inline in the Planning "Goals" tab and
// the customize-wizard goal step. Blueprint drafting idiom; reuses BPCard,
// PillBtn, Dashed, Warning, SecHead, PlanningStamp. SecHead + on-bg chrome use
// textOnBg* so it stays legible on Blueprint (light page) and every theme.
// ═══════════════════════════════════════════════════════════════
const PM_GOAL_KINDS = {
  weight:  { label: 'BODY METRIC',   glyph: '◇', blurb: 'Weight · waist · body-fat' },
  lift:    { label: 'STRENGTH LIFT', glyph: '▤', blurb: '1RM · e1RM on any lift' },
  nutri:   { label: 'NUTRITION',     glyph: '◐', blurb: 'Protein · calories · fiber' },
  vital:   { label: 'LIFESTYLE',     glyph: '◍', blurb: 'Sleep · stress · steps' },
};

const PM_GOALS_SEED = [
  { id: 'bw',   kind: 'weight', name: 'body weight',   current: '187 lb', target: '168 lb', delta: '−19 lb', date: '2026-08-20', rate: '−1.05 lb/wk', status: 'aggressive', warn: 'Rate >0.7%/wk at 187 lb is aggressive.' },
  { id: 'bench',kind: 'lift',   name: 'bench 1rm',     current: '265 lb', target: '285 lb', delta: '+20 lb', date: '2026-08-20', rate: '+1.25 lb/wk', status: 'sustainable' },
  { id: 'sleep',kind: 'vital',  name: 'sleep avg',     current: '6.4 h',  target: '7.5 h',  delta: '+1.1 h', date: '2026-08-20', rate: '+0.14 h/wk', status: 'sustainable' },
  { id: 'pro',  kind: 'nutri',  name: 'daily protein', current: '145 g',  target: '180 g',  delta: '+35 g',  date: '2026-06-01', rate: '+5 g/wk',    status: 'sustainable' },
];
const PM_GOALS_ARCHIVED = [
  { id: 'waist', kind: 'weight', name: 'waist', current: '34 in', target: '31 in', date: '2026-08-20' },
];

// One editable field, drawn as a dashed drafting input.
function PMField({ label, value, accent, w }) {
  return (
    <div style={{ width: w }}>
      <div style={{ fontSize: 7, letterSpacing: '.18em', color: BP.textTer, fontFamily: BP.fontBody }}>{label}</div>
      <div style={{
        display: 'inline-flex', alignItems: 'center', gap: 6, marginTop: 2,
        border: `1px dashed ${accent ? BP.accentBorder : BP.border}`,
        background: accent ? BP.accentFaint : 'transparent',
        padding: '3px 8px', minWidth: 0,
      }}>
        <Reenie style={{ fontSize: 17, fontWeight: 700, color: accent ? BP.accent : BP.text, lineHeight: 1 }}>{value}</Reenie>
        <svg width="9" height="9" viewBox="0 0 10 10" style={{ flexShrink: 0, opacity: .7 }}>
          <path d="M7 1 L9 3 L3.5 8.5 L1 9 L1.5 6.5 Z" fill="none" stroke={accent ? BP.accent : BP.textTer} strokeWidth="1"/>
        </svg>
      </div>
    </div>
  );
...
function GoalEditorRow({ goal, rank, total, onArchive, onUp, onDown }) {
  const statusColor = goal.status === 'aggressive' ? BP.core : goal.status === 'danger' ? BP.legs : BP.pull;
  const kind = PM_GOAL_KINDS[goal.kind];
  return (
    <BPCard style={{ margin: '10px 12px 0' }} accent={rank === 1}>
      {/* header: priority + name + reorder */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
        <div style={{
          width: 26, height: 26, flexShrink: 0,
          border: `1.5px solid ${rank === 1 ? BP.accentBorder : BP.borderStrong}`,
          background: rank === 1 ? BP.accentFaint : 'transparent',
          display: 'grid', placeItems: 'center',
        }}>
          <Reenie style={{ fontSize: 14, fontWeight: 700, color: rank === 1 ? BP.accent : BP.text }}>{rank}</Reenie>
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontFamily: BP.fontDisplay, fontSize: 12, letterSpacing: '.10em', color: BP.text }}>{goal.name}</div>
          <div style={{ fontFamily: BP.fontBody, fontSize: 7.5, letterSpacing: '.16em', color: BP.textTer, marginTop: 1 }}>{kind.glyph} {kind.label}</div>
        </div>
        {/* reorder handles */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2, flexShrink: 0 }}>
          <button onClick={onUp} disabled={rank === 1} style={{
            width: 22, height: 15, padding: 0, cursor: rank === 1 ? 'not-allowed' : 'pointer',
            background: 'transparent', border: `1px solid ${BP.border}`, color: rank === 1 ? BP.borderStrong : BP.textSec,
            fontFamily: BP.fontBody, fontSize: 9, lineHeight: 1,
          }}>▲</button>
          <button onClick={onDown} disabled={rank === total} style={{
            width: 22, height: 15, padding: 0, cursor: rank === total ? 'not-allowed' : 'pointer',
            background: 'transparent', border: `1px solid ${BP.border}`, color: rank === total ? BP.borderStrong : BP.textSec,
            fontFamily: BP.fontBody, fontSize: 9, lineHeight: 1,
          }}>▼</button>
        </div>
      </div>

      <Dashed style={{ margin: '9px 0' }}/>

      {/* current → target → delta (editable fields) */}
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, flexWrap: 'wrap' }}>
        <PMField label="CURRENT" value={goal.current}/>
        <div style={{ fontFamily: BP.fontBody, fontSize: 16, color: BP.textTer, paddingBottom: 4 }}>→</div>
        <PMField label="TARGET" value={goal.target} accent/>
        <div style={{ flex: 1 }}/>
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: 7, letterSpacing: '.18em', color: BP.textTer, fontFamily: BP.fontBody }}>DELTA</div>
          <Reenie style={{ display: 'block', fontSize: 17, fontWeight: 700, color: statusColor, marginTop: 2 }}>{goal.delta}</Reenie>
        </div>
      </div>

      <Dashed style={{ margin: '9px 0' }}/>

      {/* date + computed rate + actions */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 8 }}>
        <PMField label="TARGET DATE" value={goal.date} w="auto"/>
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: 7, letterSpacing: '.18em', color: BP.textTer, fontFamily: BP.fontBody }}>COMPUTED RATE</div>
          <Reenie style={{ display: 'block', fontSize: 12, fontWeight: 700, color: statusColor, marginTop: 2 }}>{goal.rate}</Reenie>
          <div style={{ fontSize: 8, color: statusColor, fontFamily: BP.fontBody, letterSpacing: '.1em', textTransform: 'uppercase' }}>· {goal.status}</div>
        </div>
      </div>

      {/* action row */}
      <div style={{ display: 'flex', gap: 8, marginTop: 11, paddingTop: 9, borderTop: `1px dashed ${BP.borderFaint || BP.border}` }}>
        <PillBtn size="sm" style={{ flex: 1 }}>✎ Edit</PillBtn>
        <PillBtn size="sm" ghost onClick={onArchive} style={{ flex: 1 }}>⌫ Archive</PillBtn>
      </div>
    </BPCard>
  );
}
...
function AddGoalSheet({ onClose }) {
  return (
    <div onClick={onClose} style={{
      ...
      background: BP.isDark ? 'rgba(0,0,0,.5)' : 'rgba(20,40,70,.32)',
      display: 'flex', alignItems: 'flex-end',
    }}>
      <div onClick={e => e.stopPropagation()} style={{
        width: '100%', background: BP.surface,
        borderTop: `2px solid ${BP.accent}`, padding: '14px 14px 20px',
        position: 'relative',
      }}>
        <Crosshair style={{ top: -5, left: -5 }}/>
        <Crosshair style={{ top: -5, right: -5 }}/>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
          <div style={{ fontFamily: BP.fontDisplay, fontSize: 13, letterSpacing: '.06em', color: BP.text }}>add a goal</div>
          <button onClick={onClose} style={{
            width: 26, height: 26, background: 'transparent', border: `1px solid ${BP.border}`,
            color: BP.text, fontFamily: BP.fontBody, fontSize: 13, cursor: 'pointer', lineHeight: 1,
          }}>×</button>
        </div>
        <div style={{ fontFamily: BP.fontBody, fontSize: 9, letterSpacing: '.14em', color: BP.textSec, marginBottom: 12 }}>PICK A TYPE · ENGINE COMPUTES THE RATE</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          {Object.entries(PM_GOAL_KINDS).map(([k, v]) => (
            <button key={k} onClick={onClose} style={{
              textAlign: 'left', cursor: 'pointer',
              border: `1px solid ${BP.border}`, background: BP.bgAlt,
              padding: '11px 12px', display: 'flex', flexDirection: 'column', gap: 4,
            }}>
              <div style={{ fontFamily: BP.fontDisplay, fontSize: 18, color: BP.accent, lineHeight: 1 }}>{v.glyph}</div>
              <div style={{ fontFamily: BP.fontBody, fontSize: 9.5, fontWeight: 700, letterSpacing: '.12em', color: BP.text }}>{v.label}</div>
              <div style={{ fontFamily: BP.fontBody, fontSize: 10, color: BP.textSec, lineHeight: 1.35 }}>{v.blurb}</div>
            </button>
          ))}
        </div>
      </div>
...
function ScreenGoalsEditor({ state = 'default' }) {
  const [goals, setGoals] = usePMState(state === 'empty' ? [] : PM_GOALS_SEED);
  const [archived, setArchived] = usePMState(state === 'empty' ? [] : PM_GOALS_ARCHIVED);
  const [adding, setAdding] = usePMState(state === 'add');

  const move = (i, dir) => setGoals(g => {
    const j = i + dir;
    if (j < 0 || j >= g.length) return g;
    const next = g.slice(); [next[i], next[j]] = [next[j], next[i]]; return next;
  });
  const archive = (id) => setGoals(g => {
    const found = g.find(x => x.id === id);
    setArchived(a => found ? [...a, found] : a);
    return g.filter(x => x.id !== id);
  });

  return (
    <div style={{ position: 'relative', height: '100%', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', inset: 0, bottom: 56, overflowY: 'auto', paddingBottom: 40 }}>
        {/* Header */}
        <div style={{ position: 'relative', padding: '12px 14px 10px', borderBottom: `1px solid ${BP.borderStrong}`, background: BP.bgAlt }}>
          <div style={{ position: 'absolute', top: 8, right: 12 }}><PlanningStamp small/></div>
          <button style={{
            display: 'inline-flex', alignItems: 'center', gap: 5, background: 'transparent', border: 'none',
            cursor: 'pointer', padding: 0, color: BP.textOnBgSec, marginBottom: 6,
          }} className="ft-on-bg">
            <span style={{ fontFamily: BP.fontBody, fontSize: 12 }}>‹</span>
            <span style={{ fontFamily: BP.fontBody, fontSize: 9, letterSpacing: '.14em', textTransform: 'uppercase' }}>Planning</span>
          </button>
          <div className="ft-on-bg" style={{ fontFamily: BP.fontDisplay, fontSize: 15, letterSpacing: '.05em', color: BP.textOnBg }}>goals editor</div>
          <div className="ft-on-bg" style={{ fontFamily: BP.fontBody, fontSize: 9, letterSpacing: '.18em', color: BP.textOnBgSec, marginTop: 2, fontWeight: 600 }}>
            {goals.length} ACTIVE · {archived.length} ARCHIVED · DRAG TO PRIORITIZE
          </div>
        </div>
        ...
        {goals.length > 0 ? (
          <>
            <div style={{ padding: '8px 12px 0' }}>
              <SecHead num="01" title="active goals" sub="ranked by priority · the engine weights P1 first" />
            </div>
            {goals.map((g, i) => (
              <React.Fragment key={g.id}>
                <GoalEditorRow goal={g} rank={i + 1} total={goals.length}
                  onUp={() => move(i, -1)} onDown={() => move(i, +1)} onArchive={() => archive(g.id)}/>
                {g.warn && <Warning>{g.warn}</Warning>}
              </React.Fragment>
            ))}
          </>
        ) : (
          <div style={{ padding: '16px 12px 0' }}>
            <BPCard label="NO GOALS">
              <div style={{ textAlign: 'center', padding: '14px 8px' }}>
                <div style={{ fontFamily: BP.fontDisplay, fontSize: 30, color: BP.textTer, lineHeight: 1 }}>◇</div>
                <div style={{ fontFamily: BP.fontDisplay, fontSize: 15, color: BP.text, marginTop: 8, letterSpacing: '.04em' }}>no goals set</div>
                <div style={{ fontFamily: BP.fontBody, fontSize: 11, color: BP.textSec, marginTop: 4, lineHeight: 1.4 }}>
                  Add one and the engine computes its rate against your timeline.
                </div>
              </div>
            </BPCard>
          </div>
        ...
        <div style={{ padding: '12px 12px 0' }}>
          <button onClick={() => setAdding(true)} style={{
            ...
            border: `1.5px dashed ${BP.accentBorder}`, background: BP.accentFaint,
            padding: '13px 12px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
          }}>
            <span style={{ fontFamily: BP.fontDisplay, fontSize: 16, color: BP.accent, lineHeight: 1 }}>＋</span>
            <span style={{ fontFamily: BP.fontBody, fontSize: 10, letterSpacing: '.16em', textTransform: 'uppercase', color: BP.accent, fontWeight: 700 }}>Add a goal</span>
          </button>
        </div>
        ...
        {archived.length > 0 && (
          <>
            <div style={{ padding: '18px 12px 0' }}>
              <SecHead num="02" title="archived" sub="paused · restore anytime, no data lost" />
            </div>
            <div style={{ padding: '0 12px', display: 'flex', flexDirection: 'column', gap: 8 }}>
              {archived.map(a => (
                <div key={a.id} style={{
                  display: 'flex', alignItems: 'center', gap: 10,
                  border: `1px dashed ${BP.border}`, background: BP.bgAlt, padding: '9px 11px', opacity: .85,
                }}>
                  <span style={{ fontFamily: BP.fontDisplay, fontSize: 13, color: BP.textTer }}>{PM_GOAL_KINDS[a.kind].glyph}</span>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontFamily: BP.fontDisplay, fontSize: 11, letterSpacing: '.08em', color: BP.textSec }}>{a.name}</div>
                    <div style={{ fontFamily: BP.fontBody, fontSize: 9, color: BP.textTer, marginTop: 1 }}>{a.current} → {a.target} · {a.date}</div>
                  </div>
                  <PillBtn size="sm">Restore</PillBtn>
                </div>
              ))}
            </div>
          </>
      ...
      <div style={{
        ...
        borderTop: `1px solid ${BP.borderStrong}`, background: BP.bgAlt,
        padding: '9px 12px', display: 'flex', alignItems: 'center', gap: 8,
      }}>
        <div className="ft-on-bg" style={{ fontFamily: BP.fontBody, fontSize: 9, letterSpacing: '.12em', color: BP.textOnBgSec, flex: 1 }}>
          CHANGES SAVE TO THE PLANNING DRAFT
        </div>
        <PillBtn size="sm" primary>Done ↗</PillBtn>
      </div>
