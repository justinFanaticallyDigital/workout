...
// Rail mode injection — mirrors the theme setter. The host (cluster file) wires
// its `railMode` tweak in via setActiveGameplanRailMode() each render, and sets
// window.__gpSetRailMode so the rail's own Hide/Show controls flip the tweak.
let GP_RAILMODE = 'rail';
function setActiveGameplanRailMode(m) { GP_RAILMODE = (m === 'chip' ? 'chip' : 'rail'); }
window.setActiveGameplanRailMode = setActiveGameplanRailMode;
...
// ─── GPSparkline (14-day trend) ──────────────────────────────────
function GPSparkline({ data, width = 70, height = 22, color = GP.textTer, stroke = 1.4 }) {
...
// ─── Bottom nav ────────────────────────────────────────────────
// REMOVED. The gameplan tier now renders the ONE canonical bottom shell —
// tier-homes `BottomNav({T, tier:'gameplan', activeSlot})` — injected via the
// phone navSlot in the cluster file. The old gameplan/progress/+log/nutri/settings
// strip was a second, wrong shell (Progress + Settings aren't pillars). See CLAUDE.md.

// ─── Status strip (sticky top) ─────────────────────────────────
function StatusStrip({ name = 'IRON & GROW', week = 6, totalWeeks = 16, block = 'BLOCK 2: STRENGTH', daysLeft = 71, collapsed = false }) {
  if (collapsed) {
    // Compact bar — one line: name · week · ACTIVE dot · days left. Drops the
    // spray underline, big day count and week-progress bar to reclaim ~90px.
    return (
      <div style={{
        padding: '8px 18px',
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, minWidth: 0 }}>
          <Marker style={{ fontSize: 17, color: GP.text, letterSpacing: '.01em', lineHeight: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{name}</Marker>
          <Archivo style={{ fontSize: 9, color: GP.textSec, letterSpacing: '.1em', flexShrink: 0 }}>W{week}/{totalWeeks}</Archivo>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
          <StatusDot tone="green"/>
          <Archivo style={{ fontSize: 9, color: GP.text, letterSpacing: '.1em' }}>{daysLeft} DAYS</Archivo>
        </div>
      </div>
    );
  }
...
function GPSectionH({ children, kicker, sprayWidth = 110, color = GP.info, style }) {
  return (
...
function GPGoalCard({ goal, tilt = 0, onTap }) {
  ...
  return (
    <>
      <div style={{
        ...
        {GOALS.map(g => (
          <GPGoalCard key={g.label} goal={g} tilt={g.tilt} onTap={() => setOpen(g)}/>
        ))}
      </div>
...
function TrainingTab() {
  return (
    <div style={{ padding: '16px 18px 8px' }}>
      <GPSectionH kicker="STRUCTURE" sprayWidth={130} color={GP.info}>PROGRAM MAP</GPSectionH>
      <GPBlockTimeline/>

      <div style={{ height: 18 }}/>
      <GPSectionH kicker="THIS WEEK" sprayWidth={140} color={GP.info}>SCHEDULE</GPSectionH>
      <WeekSchedule/>
...
function NutritionTab() {
  ...
  return (
    <div style={{ padding: '16px 18px 8px' }}>
      <GPSectionH kicker="TODAY" sprayWidth={120} color={GP.info}>CALORIES</GPSectionH>
      ...
      <GPSectionH kicker="MACROS" sprayWidth={110} color={GP.info}>SPLIT</GPSectionH>
...
function LifestyleTab() {
  return (
    <div style={{ padding: '16px 18px 8px' }}>
      <GPSectionH kicker="HABITS · 16-WEEK PLAN" sprayWidth={170} color={GP.info}>DAILY VITALS</GPSectionH>
      <Archivo style={{
...
// Canonical pillar layout for the Gameplan tier (CLAUDE.md mandate): the same
// PillarShell that logger + program use, fed the gameplan theme + chrome.
// StatusStrip is the pinned header; the spray-dots are the z-0 decoration; the
// rail runs the full height beside the scroll content. Pillar switching lives on
// the BottomNav (host navSlot) — the SubTabs strip is gone. Gameplan tier
// unlocks every rail slot (Today / Block / Program / Gameplan).
function GPShell({ pillar = 'training', activeKey = 'today', header, children }) {
  return (
    <PillarShell
      T={GP}
      pillar={pillar} tier="gameplan" activeKey={activeKey}
      railMode={GP_RAILMODE}
      onSetRailMode={(m) => { if (window.__gpSetRailMode) window.__gpSetRailMode(m); }}
      bg={{ ...concreteBg, overflow: 'hidden' }}
      decoration={<div style={{ position: 'absolute', inset: 0, zIndex: 0, overflow: 'hidden' }}><SprayDotsLayer seed={3}/></div>}
      header={header === undefined ? <StatusStrip/> : header}
      contentPad={84}>
      {children}
    </PillarShell>
  );
...
function ScreenMain({ openGoal = null }) {
  return (
    <GPShell pillar="training" activeKey="today">
      <div style={{ position: 'relative', padding: '14px 18px 0' }}>
        ...
        <GPSectionH kicker="UP NEXT" sprayWidth={120} color={GP.info} style={{ marginBottom: 10 }}>NEXT ACTION</GPSectionH>
        ...
        <div style={{ height: 16 }}/>
        <GPSectionH kicker="TARGETS" sprayWidth={130} color={GP.info} style={{ marginBottom: 4 }}>GOAL PULSE</GPSectionH>
      </div>
  ...
  return (
    <GPShell pillar="training" activeKey="today">
      <div style={{ padding: '14px 18px 0' }}>
        <GPSectionH kicker="DONE TODAY" sprayWidth={140} color={GP.pull} style={{ marginBottom: 10 }}>NEXT ACTION</GPSectionH>
...
function ScreenNutrition() {
  return (
    <GPShell pillar="nutrition" activeKey="today">
      <NutritionTab/>
    </GPShell>
  );
}
...
function ScreenLifestyle() {
  return (
    <GPShell pillar="lifestyle" activeKey="today">
      <LifestyleTab/>
    </GPShell>
  );
}
...
function ScreenRest() {
  return (
    <GPShell pillar="training" activeKey="today" header={<StatusStrip name="IRON & GROW" daysLeft={70}/>}>
      <div style={{ padding: '14px 18px 0' }}>
        <GPSectionH kicker="TODAY" sprayWidth={120} color={GP.info} style={{ marginBottom: 10 }}>NEXT ACTION</GPSectionH>
...
Object.assign(window, {
  ScreenMain, ScreenLogged, ScreenNutrition, ScreenLifestyle, GPScreenEmpty, ScreenRest,
